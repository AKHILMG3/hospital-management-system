...
const Reset_Password = () => {
  ...
  const submitChangePassword = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/doctor-change-password/`,
        changeData,
        {
      ...
      setChangeError(readError(err.response?.data, "Unable to change password."))
    } finally {
  ...
  const requestOtp = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/doctor-forgot-password/request-otp/`,
        { email: forgotData.email }
      )
      setOtpSent(true)
      setForgotSuccess(res.data?.message || "OTP sent to your email.")
    } catch (err) {
      setForgotError(readError(err.response?.data, "Unable to send OTP."))
    } finally {
      setLoadingOtp(false)
    }
  }
  ...
  const submitForgotReset = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/doctor-forgot-password/reset/`, payload)
      setForgotSuccess(res.data?.message || "Password reset successful.")
      setForgotData({ email: "", otp: "", new_password: "" })
      setOtpSent(false)
      setTimeout(() => logoutUser(navigate), 800)
    } catch (err) {
      setForgotError(readError(err.response?.data, "Unable to reset password."))
    } finally {
    ...
    <div style={styles.page}>
      <div style={styles.grid}>
        <div style={styles.card}>
          ...
          <p style={styles.subtitle}>For logged-in doctors</p>
